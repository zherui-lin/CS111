...
public:
	...
	: crypto_error(std::make_shared<std::string>
...
struct Key : public std::array<std::uint8_t, 32> {
	...
	SHA256(reinterpret_cast<const std::uint8_t*>(s.data()),
	       s.size(), data());
    }
...
struct PageCrypter {
    ...
    void encrypt(std::uint8_t *dst, const std::uint8_t *src,
		 std::size_t len, std::size_t offset);
    void decrypt(std::uint8_t *dst, const std::uint8_t *src,
		 std::size_t len, std::size_t offset);
