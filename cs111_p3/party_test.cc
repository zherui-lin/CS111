...
void single_sign_many(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        std::thread guest_x(guest, &party1, std::to_string(i), 2, 2,
                &matches[i]);
        guest_x.detach();
...
void random_party()
{
    ...
    for (int i = 0; i < PEOPLE; i++) {
        ...
        printf("guest %d arrives: sign %d, other_sign %d\n", i, my_sign,
                other_sign);
        std::thread guest_n(guest, &party1, std::to_string(i), my_sign,
                other_sign, &matches[i]);
        guest_n.detach();
    }
