...
[[gnu::noinline]] void
secure_erase(volatile uint8_t *data, size_t size)
{
    std::fill(data, data + size, 0);
}

[[noreturn]] void
crypto_raise(std::string msg = "")
{
...
template<typename T, typename R, R (*Destructor)(T*)> struct ctype<Destructor> {
    T *val_;
    ctype() : val_(nullptr) {}
    explicit ctype(T *val) : val_(val) {}
    ctype(ctype &&other) : val_(other.val_) { other.val_ = nullptr; }
    ~ctype() { if (val_) Destructor(val_); }
...
void
xorbuf(uint8_t *dst, const uint8_t *src1, const uint8_t *src2, size_t len)
{
    using u = uint64_t;
    static_assert(PageCrypter::blocksize % sizeof(u) == 0);
    for (size_t i = 0; i < len; i += sizeof(u))
	reinterpret_cast<u&>(dst[i]) =
	    reinterpret_cast<const u&>(src1[i]) ^
	    reinterpret_cast<const u&>(src2[i]);
}
} // namespace (anonymous)

void
PageCrypter::encrypt(uint8_t *dst, const uint8_t *src,
...
void
PageCrypter::decrypt(uint8_t *dst, const uint8_t *src,
		     size_t len, size_t offset)
{
    std::unique_ptr<uint8_t[]> buf = tweaks(offset, len);
    xorbuf(dst, src, buf.get(), len);
...
PageCrypter::tweaks(size_t offset, size_t len)
{
    ...
    CipherCtx ctx{EVP_CIPHER_CTX_new()};
    if (EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), nullptr, &key_[16],
			   nullptr) != 1)
	crypto_raise("EVP_EncryptInit_ex(aes_128_ecb)");
    int outl;
