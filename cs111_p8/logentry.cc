...
uint32_t
crc32(const void *_buf, size_t len, uint32_t crc)
{
...
inline void
mustread(Reader &r, void *buf, size_t n)
{
    if (!r.tryread(buf, n))
...
struct WithBytes {
    ...
    template<typename I> integral_t<I>
    operator()(const char *, I &i) const {
        f_(reinterpret_cast<uint8_t*>(&i), sizeof(i));
    }

    template<typename I> integral_t<I>
    operator()(const char *, std::vector<I> &v) const {
        if (v.size() > 0xff)
...
entry_nbytes(const E &e)
{
    size_t res = 0;
    const_cast<E&>(e).archive(WithBytes([&res](uint8_t *p, size_t n) {
        res += n;
    }));
    return res;
}
...
entry_read(Reader &r, E &e)
{
    e.archive(WithBytes([&r](uint8_t *p, size_t n) { mustread(r, p, n); }));
}

template<typename E> void
entry_write(Writer &w, const E &e)
{
    const_cast<E&>(e).archive(WithBytes([&w](uint8_t *p, size_t n) {
        w.write(p, n);
    }));
}

inline void
value_show(std::ostream &os, uint32_t u)
{
    os << u;
}

inline void
value_show(std::ostream &os, const std::vector<std::uint8_t> &v)
{
    os << hexdump(v.data(), v.size());
}

template<typename I> inline integral_t<I>
value_show(std::ostream &os, const std::vector<I> &v)
{
    for (I i : v)
        os << " " << i;
}
...
template<typename E> void
entry_show(std::ostream &os, const E &e)
{
    os << "  " << e.type() << "\n";
...
std::string
LogEntry::show(const filsys *sb) const
{
...
size_t
LogEntry::nbytes() const
{
    return sizeof(Header) +
        visit([](auto &e) { return entry_nbytes(e); }) +
        sizeof(Footer);
}

std::string
what_data_patch(const LogPatch &e)
{
    std::ostringstream res;
...
std::string
what_inode_patch(const LogPatch &e)
{
#define FIELD(field) { offsetof(inode, field), #field }
...
#undef FIELD
    ...
    if (e.bytes.size() >= sizeof(inode))
        ...
        if (e.bytes.size() == 2 && !(s&1) &&
            s >= offsetof(inode, i_addr) &&
...
std::string
what_patch(const filsys &sb, const LogPatch &e)
{
