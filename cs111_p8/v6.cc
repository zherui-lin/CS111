...
std::string
lsline(const struct Inode *ip, bool use_atime = false)
{
    ...
    res << " " << std::setw(3) << unsigned(ip->i_nlink) << " "
        << std::setw(3)<< unsigned(ip->i_uid) << " "
        << std::setw(3) << unsigned(ip->i_gid)
        << " " << std::setw(8) << ip->size() << " "
        << fmttime(ftime) << "  ";
    return res.str();
}
...
void
cmd_truncate(int argc, char **argv)
{
    if (argc != 2) {
...
cmd_put(int argc, char **argv)
{
    ...
    if (struct stat sb; fstat(in, &sb) == -1 ||
