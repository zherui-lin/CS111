...
struct Fsck {
    ...
    struct saved_context {
        ...
        saved_context() {}
        saved_context(Fsck *t, std::string c)
            : target(t), oldcontext(t->ctx_) { t->ctx_ = std::move(c); }
        ...
        ~saved_context() { if (target) target->ctx_ = oldcontext; }
    };
...
bool
Fsck::scan_blocks(BlockPtrArray ba, BlockPath end)
{
...
bool
Fsck::scan_directory(Ref<Inode> ip, uint16_t parent)
{
    ...
    for (Cursor c{ip}; direntv6 *p = c.next<direntv6>();) {
        ...
        if (!valid_inum(p->d_inumber)) {
            out() << "invalid inumber " << p->d_inumber << " for "
                  << name << "\n";
            res = false;
            patch(&p->d_inumber, 0);
            continue;
        ...
        if ((eip->i_mode & IFMT) == IFDIR) {
            if (nlinks_.at(p->d_inumber) != 1) {
                out() << "hard link \"" << name << "\" to directory "
                      << p->d_inumber << "\n";
                res = false;
                --nlinks_.at(p->d_inumber);
                patch(&p->d_inumber, 0);
