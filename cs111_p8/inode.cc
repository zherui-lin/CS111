...
void
Dirent::set_inum(uint16_t inum) const
{
    de_->d_inumber = inum;
    if (!inum)
        de_->name("");
...
static void
free_blocks(BlockPtrArray ba, BlockPath start)
{
...
void
Inode::truncate(uint32_t sz, DoLog dolog)
{
...
Dirent
Inode::lookup(std::string_view name)
{
...
Dirent
Inode::create(std::string_view name)
{
    if ((i_mode & IFMT) != IFDIR)
