...
V6FS::V6FS(std::string path, FScache &cache, int flags)
    : readonly_(flags & V6_RDONLY),
      fd_(::open(path.c_str(), readonly_ ? O_RDONLY : O_RDWR)),
      cache_(cache)
{
    if (fd_ == -1)
        threrror("open");
...
void
V6FS::readblock(void *mem, uint32_t blockno)
{
...
void
V6FS::writeblock(const void *mem, uint32_t blockno)
{
    if (should_crash())
        crash();
...
Ref<Inode>
V6FS::namei(std::string path, uint16_t start)
{
...
// containing garbage.
void
V6FS::bfree_freelist(uint16_t blockno)
{
    superblock().s_fmod = 1;
...
V6FS::CacheInfo
V6FS::cache_info(void *_p, size_t n)
{
    CacheInfo res;
...
V6FS::ialloc()
{
    ...
    if (superblock().s_ninode == 0) {
        ...
        for (unsigned i = 1;
             i <= end &&
                 superblock().s_ninode < array_size(superblock().s_inode);
             ++i) {
            Ref<Inode> ip = iget(i);
            if (!(ip->i_mode & IALLOC))
...
void
V6FS::log_patch(void *_p, size_t len)
{
    assert(len > 0);
