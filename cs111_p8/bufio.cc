...
inline uint32_t
lower_bound(uint32_t pos)
{
    return pos - offset(pos);
}

inline uint32_t
upper_bound(uint32_t pos)
{
...
bool
FdReader::tryread(void *_data, size_t len)
{
...
void
FdWriter::write(const void *_data, std::size_t len)
{
