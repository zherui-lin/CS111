...
template<auto Ptr,
         bool = std::is_member_function_pointer_v<decltype(Ptr)>>
struct key_helper;

template<typename K, typename V, K V::*F>
struct key_helper<F, false> {
...
class itree_entry : private detail::itree_node {
    ...
    void destroy() { if (is_linked()) unlink(); }
...
public:
    ...
    template<typename V, auto Link, typename Compare>
...
class itree_core : detail::itree_base {
	...
	return n ?
            static_cast<const V*>
	    (struct_from_field(entry_ptr,
                               static_cast<const itree_entry *>(n))) :
            nullptr;
    }
...
public:
	...
	return to_value((v->*entry_ptr).up);
    }
...
class itree
    : public itree_core<V, Link, detail::field_compare<KP, C>>
{
public:
