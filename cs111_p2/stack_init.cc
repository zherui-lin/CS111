...
sp_t
stack_init(void *_stack, size_t size, void(*start)())
{
    uintptr_t stack{uintptr_t(_stack)};

    // Bail on ridiculously small stack sizes
    if (size < 2 * (mword_size * stack_switch_height +
                    stack_alignment_divisor + mword_size))
    ...
    assert(uintptr_t(top) % stack_alignment_divisor ==
